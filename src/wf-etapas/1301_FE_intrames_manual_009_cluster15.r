...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  # Aca agregamos los clusters usando Kmeans ++
  
  
  cat("Aca empieza a correr K medias. Suerte.\n")
  datasetsinNA <- dataset
  datasetsinNA[is.na(datasetsinNA)] <- 0
  datasetsinNA <- as.data.frame(datasetsinNA)
  datasetsinNA[,1:154] <- scale(datasetsinNA[,1:154])
  kmedias <- KMeans_rcpp(datasetsinNA[,1:154], clusters = 15, num_init = 1, initializer = "kmeans++")
...
cat( "z1301_FE_intrames_manual_009_cluster15.r  START\n")
action_inicializar() 
...
cat( "z1301_FE_intrames_manual_009_cluster15.r  END\n")
